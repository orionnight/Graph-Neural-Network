...
def test(epoch,tes, wht):
    ...
    prob_pred = F.softmax(output).cpu().data.numpy()
    ...
    return loss_test.data[0], acc_test1.data[0], acc_test2.data[0],prob_pred    


#-------------------------- Load Model and optimizer --------------------------

path_train = '../../py_mind_tr_oas'
path_valid = '../../py_mind_va_oas'
path_test  = '../../py_mind_te_oas'
...
for in_wh in range(len(wht_file)):

    model = torch.load(path + wht_file[in_wh])
    mkdir('./output_' + wht_file[in_wh])
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    #print(sum(p.numel() for p in model.parameters()))

    #model = torch.load(path + 'weights_187')
    #optimizer = optim.Adam(model.parameters(), lr=args.lr)
    #print(sum(p.numel() for p in model.parameters()))
    ...
    for tes in range(len(file_test)):
        ...
        loss, acc1, acc2, prob_pred = test(0,tes, wht)

        gt = torch.max(labels, 1)[1].cpu().data.numpy()
        labels = labels.cpu().data.numpy()

        sio.savemat('./output_' + wht_file[in_wh] + '/'+ file_test[tes],{'prob_pred':prob_pred,'gt':gt, 'labels':labels})
        mn_ls.append(loss)
        mn_ac1.append(acc1)
        mn_ac2.append(acc2)
