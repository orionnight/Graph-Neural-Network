...
def test(epoch,tes, wht):
    ...
    return loss_test.data[0], acc_test1.data[0], acc_test2.data[0],np.round(prob_pred,4)    
...
for tes in range(len(file_test)):
    ...
    sio.savemat('./output_best/'+ file_test[tes],{'prob_pred':prob_pred,'gt':gt, 'labels':labels})
    mn_ls.append(loss)
    mn_ac1.append(acc1)
    mn_ac2.append(acc2)
